...
// Number of colors pushed by SetColor
constexpr int kColorCount = 57;

namespace Style {
    ImVec4 ToVec4(const ImColor& color) {
        return ImVec4(color.Value.x, color.Value.y, color.Value.z, color.Value.w);
    }

    ImVec4 Brighten(const ImVec4& col, float factor) {
        return ImVec4(
    ...
    ImVec4 Darken(const ImVec4& col, float factor) {
        return ImVec4(col.x * (1.0f - factor), col.y * (1.0f - factor), col.z * (1.0f - factor), col.w);
    }

    ImVec4 WithAlpha(const ImVec4& col, float alpha) {
