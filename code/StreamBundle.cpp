...
void StreamBundle::readStandaloneStreamBundleFile(char* fileName) {
...
void StreamBundle::readStreamBundleFile(char* data) {
	...
	for (int32_t i = 0; i < header.totalFileTotal; i++) {
		...
		if (magic == 0x57424E44) {
			...
			bundleFiles[i].waveBankFile = new DNBW();
			...
			bundleFiles[i].bundleFile = new BundleV36(); 
...
char* StreamBundle::getFileData(int32_t fileIdx) {
	...
	memcpy(data, data + fileEntries[fileIdx].offset, fileEntries[fileIdx].dataSize);
